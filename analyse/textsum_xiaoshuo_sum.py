...
def get_textrank(filename, filtername):
    ...
    with codecs.open(filename, 'r', 'gbk') as f:
    	for line in f.readlines():
            ...
            desc = re.sub(parten, "",data[-1]).replace(r'<br/>','')
            ...
            if len(summary) == 0:
                summary = data[1]
            newline += data[0] + '\t' + data[1] + '\t' + summary + '\t' + data[-1] + '\n'
...
def get_summary(title, data, filters):
    ...
    for i in xrange(len(items)):
        ...
        for j in xrange(len(items) - i - 1):
            if filter_sentences(items[i + j + 1].sentence, filters) and len(items[i + j + 1].sentence) != 0 and length + len(items[i + j + 1].sentence) <= 42:
    ...
    if len(summary) <= 12:
        ...
        for item in items:
            if filter_sentences(item.sentence, filters) and len(item.sentence) >= 15 and len(item.sentence) <= 32:
...
def get_summary_douhao(title, data, filters):
    ...
    for i in xrange(len(items)):
        ...
        if filter_sentences(items[i].sentence, filters) and len(items[i].sentence) != 0 and len(items[i].sentence) <= 30:
            ...
            break
        for j in xrange(len(items) - i - 1):
            if filter_sentences(items[i + j + 1].sentence, filters) and len(items[i + j + 1].sentence) != 0 and length + len(items[i + j + 1].sentence) <= 36:
                if preindex - 1 == items[i + j + 1].index:
                    summary = items[i + j + 1].sentence + '，' + summary
                    preindex -= 1
                elif index + 1 == items[i + j + 1].index:
